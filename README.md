...
# CV-распознание справок

## Задача проекта

Задачей нашего проекта является создание наиболее универсального и эффективного решения для оцифровке таблицы с данными о донации крови с непредобработанной фотографии листа A4. Тем самым минимизировать человеческие усилия.

Ранее доступные библиотеки не подходили нам из-за специфики фотографий.

## Алгоритм решения
- Предобработка фото
- Найти область основной таблицы
- Выделмть таблицу и повернуть ее
- Найти местоположения текста
- Найти иерархию ячеек
- Распознать текст с помощью tesseract-ocr
- Сформировать итоговую таблицу

## Краткий вывод

У нас вышло получить сервис на docker`e с Accuracy = 87%. В начале для повышения эффективности модели предлагается выбрать является изображение сканом или фото. Алгоритм четко определяет нераспознанные ячейки, что исключает трату времени на проверку распознанных ячеек.
